...
const Tutorials = () => {
  ...
  return (
    <div >
      <div className="px-4 pt-8 pb-20 bg-gray-700">
        ...
        {cardData.map((card, index) => (
          ...
          >
            ...
            <div className="p-3 md:p-4 w-full border-t border-gray-500 h-[42%]">
              <h2 className="text-lg md:text-xl font-semibold mb-1 md:mb-2 text-white">{card.heading}</h2>
              <p className="text-sm md:text-base text-gray-300 mb-1 md:mb-2 ">{card.about}</p>
              <p className="text-xs md:text-sm text-gray-400">Released: {card.date}</p>
            </div>
          </div>
        ))}
      </div>
