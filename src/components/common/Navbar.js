...
import { logout } from '../../services/operations/authAPIs'; // Assuming this is where your logout action is defined
...
function Navbar() {
  ...
  return (
    <nav className={`${isScrolled ? 'bg-gray-900' : 'bg-black'} w-full top-0 start-0 border-b-2`}>
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4 sm:space-y-2">
        ...
        <div className="flex md:order-2 space-x-3 md:space-x-0  rtl:space-x-reverse">
          {!token ? ( // Render login and signup buttons if not logged in
            ...
            <div className='flex gap-2 pt-2'>
              <Link to="/dashboard/my-profile">
                <button type="button" className="text-white focus:outline-none font-bold rounded-xl text-sm px-3 py-2 text-center hover:bg-gray-700 hover:border-gray-500 hover:scale-105 border border-white hover:text-blue-500 ">Dashboard</button>
              </Link>
              <button type="button" onClick={handleLogout} className="text-white focus:outline-none font-bold rounded-xl text-sm px-3 py-2 text-center hover:bg-gray-700 hover:border-gray-500 hover:scale-105 border border-white hover:text-blue-500">Logout</button>
            </div>
          )}
